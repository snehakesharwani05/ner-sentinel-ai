...
import { AlertPinConfig } from "../components/RadialSubscriptionModal";
import { API_BASE_URL } from "../api/api";

const COOLDOWN_WINDOW_MS = 4 * 60 * 60 * 1000; // 4 Hours

export function useProximitySmsWatcher(
  disruptions: any[],
  pinConfig: AlertPinConfig
) {
  ...
  useEffect(() => {
    ...
    const cooldownRegistry: Record<string, number> = rawLog ? JSON.parse(rawLog) : {};
    const now = Date.now();
