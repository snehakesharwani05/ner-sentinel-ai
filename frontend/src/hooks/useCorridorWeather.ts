...
export interface ComprehensiveWeather {
  ...
  soilMoisture: number; // m³/m³ (Geotechnical saturation indicator)
  humidity: number;
  windSpeed: number;
...
export function useCorridorWeather(lat: number, lng: number, radiusKm: number = 75) {
  const [data, setData] = useState<ComprehensiveWeather | null>(null);
  ...
  useEffect(() => {
    ...
    async function fetchAllTelemetry() {
      ...
      let threat: ComprehensiveWeather["threatLevel"] = "GREEN_CLEAR";
