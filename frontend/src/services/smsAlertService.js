...
import { calculateHaversineDistance, extractDisruptionCoordinates } from "../utils/geoProximity";
 ...
 */
export async function sendBackendPooledSmsAlert(payload) {
  try {
...
export async function sendFast2SmsAlert(
  payload,
  apiKey = (typeof import.meta !== 'undefined' && (import.meta.env?.VITE_FAST2SMS_API_KEY || import.meta.env?.VITE_SMS_API_KEY)) || ""
) {
 ...
 */
export function getCooldownKey(recipientPhone, corridorName) {
 ...
 */
export function isAlertInCooldown(recipientPhone, corridorName) {
  try {
 ...
 */
export function markAlertDispatched(recipientPhone, corridorName) {
  try {
    const key = getCooldownKey(recipientPhone, corridorName);
