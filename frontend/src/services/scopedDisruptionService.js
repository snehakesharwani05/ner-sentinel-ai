...
import { NER_STATES, ALL_NER_REGION } from "../constants/nerLocations.js";
import { 
  processAndVerifyIncidents, 
  clusterTomTomIncidents, 
  getNextTomTomKey 
} from "./liveDisruptionService.js";
...
export async function fetchStateScopedDisruptions(
  activeStateId = "ALL",
  tomtomKey = getNextTomTomKey()
) {
  const isAll = !activeStateId || activeStateId === "ALL";
  const stateMeta = isAll
    ? ALL_NER_REGION
 ...
 */
export async function fetchStateScopedEarthquakes(activeStateId = "ALL") {
  const isAll = !activeStateId || activeStateId === "ALL";
  const stateMeta = isAll
    ? ALL_NER_REGION
    : (NER_STATES.find(s => s.id === activeStateId) || NER_STATES[0]);
 ...
 */
export function sortByCityProximity(incidents, cityCoords) {
